...
//
// This file is textually included by `control.rs`, so it intentionally shares
// the runtime-control module's private request/correlation primitives.
        ...
        match first {
            CommandSignal::Response(response) => match response.status.as_str() {
                "accepted" => {
                    ...
                    // Closing is made visible while this request still owns the
                    // request gate. No pause/reset request can slip in after the
                    // clean exit has been acknowledged.
