...
export type MameBrowserFilterNavItem = MameBrowserFilterDefinition | MameBrowserDeferredFilterDefinition;
...
export function filterRequiresValue(filter: MameBrowserFilter): boolean {
  return MAME_BROWSER_FILTERS.some((definition) => definition.id === filter && definition.valueKind !== undefined);
}

export function buildMameBrowserRequest(filter: MameBrowserFilter, text: string, filterValue: string, offset = 0, preferredMachine: string | null = null): MameUiMachineSearchRequest {
  return { text: text.trim() || null, filter, filterValue: filterValue.trim() || null, preferredMachine, limit: MAME_BROWSER_PAGE_SIZE, offset: Math.max(0, offset) };
}

export function reconcileMachineSelection(items: MachineListItem[], current: MachineListItem | null, preferredMachine: string | null): MachineListItem | null {
  if (current) {
...
/** Compute a page movement from the actual visible list geometry, retaining one row of context. */
export function browserPageStep(viewportHeight: number, rowHeight: number): number {
  if (!Number.isFinite(viewportHeight) || !Number.isFinite(rowHeight) || viewportHeight <= 0 || rowHeight <= 0) return 1;
  return Math.max(1, Math.floor(viewportHeight / rowHeight) - 1);
}

/** Home/End intentionally address the current fetched catalog page; pager controls cross page boundaries. */
export function nextBrowserIndex(key: string, current: number, count: number, pageStep = 1): number | null {
  if (count <= 0) return null;
  const bounded = Math.min(Math.max(current, 0), count - 1);
  switch (key) {
    case "ArrowDown": return Math.min(count - 1, bounded + 1);
    case "ArrowUp": return Math.max(0, bounded - 1);
    case "Home": return 0;
    case "End": return count - 1;
    case "PageDown": return Math.min(count - 1, bounded + Math.max(1, pageStep));
    case "PageUp": return Math.max(0, bounded - Math.max(1, pageStep));
    default: return null;
  }
}
