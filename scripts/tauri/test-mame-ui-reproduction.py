...
def main() -> int:
    ...
    for legacy_panel in ("GeneralSettingsPanel", "DiagnosticsPanel", "SessionControlPanel", "BulkAuditPanel", "LibraryBrowser", "RecentHistoryPanel", "CollectionManager"):
    ...
    require(right_panel, "setAssetState(initialArtworkAssetState(Boolean(descriptor)))", "explicit artwork asset lifecycle")
    require(right_panel, "errorMessage(reason)", "shared artwork error formatter")
    require(controller, "Inherit global selection", "machine/global controller distinction")

    for token in ("queryMameSoftware", "queryMameBiosChoices", "Start Empty", "Launch part", "onBack", 'event.key === "Escape"'):
        require(software, token, "contextual software browser parity")
    for token in ("gameplayInputOwned", 'role="listbox"', 'role="option"', "aria-selected={isSelected}", "tabIndex={isSelected ? 0 : -1}", "launchMameEmpty", "bios: selectedBios"):
        require(software, token, "post-closure software-browser hardening")
    require(frontend_mame_software, 'invoke<SessionSnapshot>("launch_mame_empty"', "typed Start Empty invocation")
    ...
    parity_styles = {"App.css": app_css, "MameShell.css": shell_base_css, "ContextualSurfaces.css": shell_css, "SoftwareBrowser.css": software_css}
    ...
    for token in ("var(--mame-bg)", "var(--mame-text)", "var(--mame-border)", "var(--mame-selected)", "var(--mame-selected-text)", "var(--mame-focus)", "var(--mame-disabled)"):
    ...
    for task_number in range(1, 21): require(todo, f"MUI-{task_number:03d}", "TODO task inventory")
    if any(line.lstrip().startswith("- [ ]") for line in todo.splitlines()): raise SystemExit("MAME UI reproduction regression: reconciled milestone TODO has unchecked items")
    ...
    for task_number in range(1, 8): require(remediation_todo, f"MUR-{task_number:03d}", "post-closure remediation task inventory")
    if any(line.lstrip().startswith("- [ ]") for line in remediation_todo.splitlines()): raise SystemExit("MAME UI reproduction regression: remediation TODO has unchecked items")
    for token in ("Category", "Custom Filter", "DAT", "Software Favorites"): require(parity + closure, token, "explicit parity disposition")
    for token in ("Keyboard-only path", "Focus visibility and restoration", "Controller/gamepad navigation disposition"): require(accessibility, token, "accessibility qualification")
    for token in ("Displayed-list export", "Deliberate machine-filter defers", "Deliberate software defers", "Security invariants preserved"): require(closure, token, "closure record")
    for required_path in ("/docs/MAME_TAURI_MAME_UI_REPRODUCTION_SPEC_2026-09-14.md", "/docs/MAME_TAURI_MAME_UI_REPRODUCTION_TODO_2026-09-14.md", "/docs/MAME_TAURI_MAME_UI_PARITY_MATRIX_2026-09-14.md", "/docs/MAME_TAURI_MAME_UI_ACCESSIBILITY_QUALIFICATION_2026-09-15.md", "/docs/MAME_TAURI_MAME_UI_REPRODUCTION_CLOSURE_2026-09-15.md", "/docs/MAME_TAURI_UI_POST_CLOSURE_REMEDIATION_TODO_2026-09-14.md"):
